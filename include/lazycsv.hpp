...
public:
  ...
  auto operator*() const { return T{begin_, end_}; }
...
private:
  constexpr static auto is_trim_char(char) { return false; }

  template <class... Other_chars>
  constexpr static auto is_trim_char(char c, char trim_char,
...
public:
  ...
  public:
    ...
    auto raw() const { return std::string_view(begin_, end_ - begin_); }
  ...
  private:
    static const auto *escape_leading_quote(const char *begin,
                                            const char *end) {
    ...
    static const auto *escape_trailing_quote(const char *begin,
                                             const char *end) {
      if (end - begin >= 2 && *begin == quote_char::value &&
          *(end - 1) == quote_char::value)
        return end - 1;
  ...
  public:
    ...
    template <typename... Indexes> auto cells(Indexes... indexes) const {
    ...
    auto begin() const { return cell_iterator{begin_, end_}; }

    auto end() const { return cell_iterator{end_ + 1, end_ + 1}; }
  };
